  ...
  const handleBackup = async () => {
    ...
    try {
      ...
      setModalState({
        ...
        type: "error",
        title: "Error",
        message: err instanceof Error ? err.message : "Failed to backup database",
      })
    } finally {
...
${result.summary.skipped_details.length > 0 ? `Skipped:\n${result.summary.skipped_details.join('\n')}` : ''}`
      ...
      setModalState({
        ...
        type: "error",
        title: "Error",
        message: err instanceof Error ? err.message : "Failed to restore database",
      })
    } finally {
      onLoadingChange(false)
    }
        ...
        <CollapsibleContent>
          <div className="border-t-2 border-black p-6 space-y-6">
            ...
            <div className="space-y-3">
              <h3 className="font-mono font-semibold text-lg">Backup Database</h3>
            ...
            {/* Restore Section */}
            <div className="space-y-3">
              <h3 className="font-mono font-semibold text-lg">Restore Database</h3>
              <p className="text-sm text-gray-600">
                Upload a backup ZIP file to restore database records. This will append data to existing tables.
              </p>
